...
public class DeviceDataServiceImpl implements DeviceDataService {
    ...
    @Override
    public List<DeviceDataVo> readDeviceDataByStr(String id) {
        List<DeviceData> dd =  deviceDataMapper.selectByIotId(id);
        List<DeviceDataVo> dds = new ArrayList<>();
        for (int i = 0; i < dd.size(); i++) {
            DeviceDataVo dv = BeanUtil.toBean(dd.get(i), DeviceDataVo.class);
            dds.add(dv);
        }
//        List<DeviceDataVo> dd = BeanUtil.copyToList(dds, DeviceDataVo.class);
        return dds;
    }
